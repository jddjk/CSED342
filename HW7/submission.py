...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        col_count = self.belief.getNumCols()
        row_count = self.belief.getNumRows()
        updated_probs = [[0 for _ in range(col_count)] for _ in range(row_count)]
        
        for col_index in range(col_count):
            for row_index in range(row_count):
                x_coord = util.colToX(col_index)
                y_coord = util.rowToY(row_index)
                calculated_distance = math.sqrt((agentX - x_coord) ** 2 + (agentY - y_coord) ** 2)
                probability_density = util.pdf(calculated_distance, Const.SONAR_STD, observedDist)
                prior_probability = self.belief.getProb(row_index, col_index)
                updated_probability = prior_probability * probability_density
                updated_probs[row_index][col_index] = updated_probability
        
        for col_index in range(col_count):
            for row_index in range(row_count):
                self.belief.setProb(row_index, col_index, updated_probs[row_index][col_index])
        
        for col_index in range(col_count):
            for row_index in range(row_count):
                current_prob = self.belief.getProb(row_index, col_index)
                self.belief.setProb(row_index, col_index, current_prob)
    ...
    def elapseTime(self) -> None:
        ...
        row_total = self.belief.getNumRows()
        col_total = self.belief.getNumCols()
        proposed_belief = util.Belief(row_total, col_total, 0)
        transitional_probs = [[0 for _ in range(col_total)] for _ in range(row_total)]
        
        for (initial_tile, resulting_tile), transition_probability in self.transProb.items():
            initial_row, initial_col = initial_tile
            resulting_row, resulting_col = resulting_tile
            initial_probability = self.belief.getProb(initial_row, initial_col)
            resultant_probability = initial_probability * transition_probability
            transitional_probs[resulting_row][resulting_col] += resultant_probability
        
        for row_index in range(row_total):
            for col_index in range(col_total):
                proposed_belief.addProb(row_index, col_index, transitional_probs[row_index][col_index])
        
        for row_index in range(row_total):
            for col_index in range(col_total):
                current_prob = proposed_belief.getProb(row_index, col_index)
                proposed_belief.setProb(row_index, col_index, current_prob)
        
        proposed_belief.normalize()
        self.belief = proposed_belief
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        reweighted_particles = collections.defaultdict(float)
        
        for loc in old_particles:
            calculated_dist = math.sqrt((agentX - util.colToX(loc[1])) ** 2 + (agentY - util.rowToY(loc[0])) ** 2)
            density_prob = util.pdf(calculated_dist, Const.SONAR_STD, observedDist)
            reweighted_particles[loc] = old_particles[loc] * density_prob
        
        temp_particles = collections.defaultdict(float)
        for loc in reweighted_particles:
            temp_particles[loc] = reweighted_particles[loc]

        new_particles = collections.defaultdict(int)
        for _ in range(self.NUM_PARTICLES):
            chosen_sample = util.weightedRandomChoice(temp_particles)
            new_particles[chosen_sample] += 1
    ...
    def elapseTime(self) -> None:
        ...
        intermediateParticles = collections.defaultdict(int)
        ...
        for particle in self.particles:
            ...
            if particle in self.transProbDict:
                ...
                for _ in range(count):
                    ...
                    intermediateParticles[sample] += 1
        
        for particle, count in intermediateParticles.items():
            newParticles[particle] += count
        
        finalParticles = collections.defaultdict(int)
        for particle, count in newParticles.items():
            finalParticles[particle] += count

        self.particles = finalParticles
        # END_YOUR_CODE
